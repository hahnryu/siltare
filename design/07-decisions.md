...
## 2026-03-05 - 대시보드 관리 기능

**결정:** 대시보드에 개발/운영용 액션 버튼 추가.

**추가된 기능:**
- **초고 생성 버튼:** `/api/chapter-complete` 직접 호출
- **컨텍스트 초기화 버튼:** `chapter_context` null인 인터뷰에 기본값 주입

**용도:**
- 새 코드 배포 전 생성된 인터뷰 레거시 처리
- 개발 중 테스트 데이터 초기화

**구현:**
- `/api/init-chapter-context` 엔드포인트
- `/api/interviews` GET 엔드포인트 (실시간 데이터 연동)
- 대시보드 "액션" 컬럼에 조건부 버튼 표시
